...
class TestMaksukortti(unittest.TestCase):
    ...
    def test_syo_maukkaasti_ei_vie_saldo_negativiiseksi(self):
        kortti = Maksukortti(300)
        kortti.syo_maukkaasti()

        self.assertEqual(kortti.saldo_euroina(), 3.0)

    def test_negatiivisen_summan_lisaaminen_ei_muuta_saldoa(self):
        self.kortti.lataa_rahaa(-100)
        
        self.assertEqual(self.kortti.saldo_euroina(), 10)

    def test_pystyy_syoda_edullisesti_kun_saldo_on_hinnan_verran(self):
        kortti = Maksukortti(250)
        kortti.syo_edullisesti()

        self.assertEqual(kortti.saldo, 0)

    def test_pystyy_syoda_maukkaasti_kun_saldo_on_hinnan_verran(self):
